import React from "react";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { useBuyerAuth } from "../../context/BuyerAuthContext";

const NAV_SECTIONS = [
  {
    title: "Overview",
    items: [
      {
        key: "dashboard",
        label: "Dashboard",
        path: "/fasalrath/buyer/dashboard",
        icon: "🏠",
      },
      {
        key: "profile",
        label: "My Profile",
        path: "/fasalrath/buyer/profile",
        icon: "👤",
      },
    ],
  },
  {
    title: "Market",
    items: [
      {
        key: "marketplace",
        label: "Marketplace",
        path: "/fasalrath/buyer/marketplace",
        icon: "🛒",
      },
      {
        key: "bidding",
        label: "Bidding",
        path: "/fasalrath/buyer/bidding",
        icon: "⚖️",
      },
    ],
  },
  {
    title: "Requirements",
    items: [
      {
        key: "requirements",
        label: "My Requirements",
        path: "/fasalrath/buyer/requirements",
        icon: "📋",
      },
      {
        key: "post-requirements",
        label: "Post Requirement",
        path: "/fasalrath/buyer/post-requirements",
        icon: "➕",
      },
    ],
  },
];

export default function BuyerPortalSidebar() {
  const { t } = useTranslation();
  const location = useLocation();
  const navigate = useNavigate();
  const { buyer, logout } = useBuyerAuth();

  const initial = buyer?.name
    ? buyer.name
        .split(" ")
        .map((w) => w[0])
        .slice(0, 2)
        .join("")
        .toUpperCase()
    : "B";

  return (
    <aside className="fr-sidebar">
      <div className="fr-sidebar-user">
        <div
          className="fr-sidebar-avatar"
          style={{ background: "linear-gradient(135deg, #2A9D8F, #21867A)" }}
        >
          {initial}
        </div>
        <div className="fr-sidebar-name">{buyer?.name || t("Buyer")}</div>
        <div className="fr-sidebar-phone">
          +91 {buyer?.phone?.replace("+91", "") || ""}
        </div>
        <div
          className="fr-sidebar-badge"
          style={{ background: "rgba(42, 157, 143, 0.1)", color: "#2A9D8F" }}
        >
          🛒 {t("Buyer")}
        </div>
      </div>

      <nav className="fr-sidebar-nav">
        {NAV_SECTIONS.map((section, si) => (
          <div key={section.title}>
            {si > 0 && <div className="fr-sidebar-divider" />}
            <div className="fr-sidebar-section">{t(section.title)}</div>
            {section.items.map((item) => (
              <Link
                key={item.key}
                to={item.path}
                className={`fr-sidebar-link ${location.pathname === item.path ? "active" : ""}`}
              >
                <span className="fr-sidebar-link-icon">{item.icon}</span>
                {t(item.label)}
              </Link>
            ))}
          </div>
        ))}

        <div className="fr-sidebar-divider" />
        <button
          className="fr-sidebar-link"
          onClick={() => {
            logout();
            navigate("/fasalrath/buyer/login");
          }}
          style={{ color: "#e76f51" }}
        >
          <span className="fr-sidebar-link-icon">🚪</span>
          {t("Logout")}
        </button>
      </nav>
    </aside>
  );
}
